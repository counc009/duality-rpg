...
\begin{itemize}
  ...
  \item \textbf{Innate}: This is an \emph{innate weapon}; choose that either its range is either melee or ranged; the item grants 1 damage die but no bonus to attack.
\end{itemize}
...
\begin{itemize}
  ...
  \item An additional damage die (1d8 for melee attacks and 1d6 for ranged) can be added for $3 \times$ the new number of damage dice on that type of attack XP (so a second d8 for a Melee weapon costs 6 XP). A weapon can have a maximum of 3 damage dice.
    A d6 damage die on a ranged attack can also be increased to a d8 for 1 XP (paid for each damage die, so a Ranged weapon dealing 2d8 damage costs 8 XP, 2 for increasing the d6 to d8 and 6 for the second damage die).
\end{itemize}
