...
On a creature's turn, they can move based on their \Speed{}\index{Speed}, take an \textbf{Action}\index{Action}, any take any reasonable number of \textbf{Quick Actions}\index{Action!Quick Action}.
...
The target subtracts their \Block{}\index{Block} from this number and any remaining damage is subtracted from its \Life{}\index{Life}.
...
\subsection{Weapon Properties}\index{Weapon}
Properties of a weapon, or unarmed or magical attacks, such as damage and range are defined by keywords on the weapon.
The keywords and their properties are detailed in Table~\ref{tab:weapons}.
Note that the attack roll for any melee attacks\index{Melee Attack} (attacks against an adjacent creature) is \emph{Decreased} if the weapon does not have the \emph{Melee} keyword.
Similarly, any ranged attack\index{Ranged Attack} (any attack against a target not adjacent) without the \emph{Ranged} keyword and any attack beyond a \emph{Mid-Range} are \emph{Decreased}.

\begin{table}
  \centering
  \begin{tabular}{|c|l|}\hline
    \textbf{Keyword} &\textbf{Properties} \\ \hline
    Melee\index{Melee|see{Weapon}} &Can be used to make a melee attack, dealing 1d8 base damage \\
    Ranged\index{Ranged|see{Weapon}} &Can be used to make a ranged attack, dealing 1d6 base damage \\
    Accurate\index{Accurate|see{Weapon}} &Increases the base damage of a \emph{Ranged} weapon to 1d8 \\
    Defensive\index{Defensive|see{Weapon}} &Grants a +1 bonus to \Block \\
    Multihit\index{Multihit|see{Weapon}} &Attacker can spend advantage to target a creature and all those adjacent to it at -1 damage dice \\
    Reach\index{Reach|see{Weapon}} &Increases the range of a melee attack to a \emph{Near} distance \\
    Far Shooter\index{Far Shooter|see{Weapon}} &Increases the range of a ranged attack to \emph{Far} \\
    Piercing\index{Piercing|see{Weapon}} &Attacker can spend advantage to ignore the target's \Block \\ \hline
  \end{tabular}
  \caption{Weapon Keywords\index{Weapon}}
  \label{tab:weapons}
\end{table}
...
The GM should determine the \Skill{} to roll for healing based on the activities undertaken to heal.
...
\begin{itemize}
  ...
  \item[-] \textbf{Unstable}: the creature remains alive but barely, with its fate hanging in the balance.
    ...
    At the end of each of its turns, it rolls 1d20 and must roll a \Mixed{} or better result with the \Difficulty{} of this roll initially \emph{Very Easy} and increasing by one level each round and each time the creature takes damage while in this state.
