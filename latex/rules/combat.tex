...
Using a Defend Action also allows the character to activate a defensive style which can provide additional bonuses like to the roll or other statistics.
...
\begin{enumerate}
  \item The PC spends a Main Action and identifies the NPC (or NPCs) they are targeting, the weapon and the supported offensive style they are using, and whether they are using any special features (such as expending additional Main Actions).
  \item The GM determines, for each NPC being targeted, whether they will expend a Defend Action and if so the Defensive Style they are using. The NPC will have a different difficulty to hit based on whether or not they use a Defend Action (generally the difficulty to hit is $-1$ if the NPC did not use a Defend Action).
...
When a PC deals damage to an NPC, the damage is determined by their Offensive Style and their Weapon.
...
\begin{enumerate}
  ...
  \item Each PC being targeted determines whether they will expend a Defend Action and if so the Defensive Style they are using.
    \begin{enumerate}
      \item If a PC does not use a Defend Action, the NPC automatically hits (unless the PC has the armored defensive style)\footnotemark and the NPC deals damage to them.
      ...
      \item If a PC does use a Defend Action they determine the defensive style they are using and any associated weapon they are using.
        A PC may also have multiple Defensive Styles that could be helpful, they may spend an Advantage to receive the benefits (other than benefits to the roll) of additional Defensive Styles.
        The PC then gets to roll to avoid the hit using the Basic Statistic of their selected defensive style and any bonuses it or their weapon grant, the difficulty of this roll is determined by the NPC's statistics.
