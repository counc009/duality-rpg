...
The attacker chooses one of their Offensive \emph{Combat Styles} for the attack and rolls the specified \Attribute{} plus any bonus for the \emph{Combat Style} and Weapon if they are using one associated with that \emph{Combat Style}.
The target chooses one of their Defensive \emph{Combat Styles} and rolls the specified \Attribute{} plus any bonus for the \emph{Combat Style} and Weapon if they are using one associated with that \emph{Combat Style}.
A target may have multiple Defensive \emph{Combat Styles} that could be useful to them; before rolling they may choose to use multiple by spending an \Advantage{} for each additional they want to activate; if doing this they must choose one which is used for the roll to defend and bonuses to the same statistics from multiple \emph{Combat Styles} do not stack.
...
If the attacker's result is \Mixed{} or better, they roll damage based on the \emph{Combat Style} and any relevant Weapon and add the same \Attribute{} bonus as on the attack (though they do not add the other bonuses to the attack).
...
If the result is \Mixed{} or worse then the target has the chance to strike back if it reasonably can; if it does it deals damage in the same manner (though does not add any damage dice from a \emph{Combat Style} and add the \Attribute{} they rolled to defend unless another rule specifies otherwise; they may add dice from a Weapon), alternatively it can take \Advantage{} as usual.
