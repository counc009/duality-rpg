...
\begin{itemize}
  ...
  \item[-] When performing triage, a character can spend an additional Main Action to target a group of creatures that are clumped together within melee range of each other.
...
The damage that an NPC does to a PC on a Mixed or worse result is determined by the NPC's statistics, and is generally less than they would deal on an attack against a PC.
...
The damage a PC does to an NPC on a Mixed or better result is determined by any damage dice granted by their Weapon and their bonus to avoid the attack granted by the Defensive Combat Style they used; it does not include any dice granted by their Offensive Combat Style.
...
To attempt healing in the midst of combat, the healer must use a Main Action and spend an \Advantage{} and must have a relevant \Specialization{} that can be used to help heal.
