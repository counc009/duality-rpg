...
\begin{table}
  ...
  \begin{tabular}{|c|l|} \hline
    ...
    None &nothing \\ \hline
    Limited &basic food and lodging daily \\ \hline
    Modest &basic (though sturdy) supplies like rope, arrows, and tents \\ \hline
    Comfortable &rare supplies like climbing equipment, a day of unskilled labor, a beast of burden \\ \hline
    Luxurious &a sailboat, a comfortable wagon, a day of skilled labor \\ \hline
    Aristocratic &a mercenary, an abandoned warehouse or small property \\ \hline
  \end{tabular}
...
Each \Experience{} has a short, couple word, description and a bonus attached to; when a character is attempting something related to one of their \emph{Experiences} they can add the \emph{Experience's} bonus to their roll.
...
While not solely for non-magical characters, these \Ability{} are designed to assist non-magical builds by offering additional options for play and ways to gain \Advantage{}.
...
\begin{itemize}
  ...
  \item \textbf{Draw Blood} ($n$ XP): When you hit a target with an attack, you can spend two \Advantage{} to inflict a \emph{bleed} on the target. The target takes $n$ damage immediately and at the beginning of each of their turns until they or an ally use an action or spend an \Advantage{}, on their turn, to end the condition.
\end{itemize}

\subsection{Combat Styles}\label{sec:combat-styles}
\emph{Combat Styles}\index{Combat Style} define the character's abilities in combat, including their bonuses to attack and defend, their range for attacks, their damage, and other special abilities.
Combat Styles are divided into \emph{Offensive} and \emph{Defensive} styles.
Each character gets one Offensive and one Defensive style for free and then any additional style costs 3 XP.
...
\subsubsection{Offensive Combat Styles}\index{Combat Style!Offensive}
\paragraph{Melee}
...
Attacks with this style deal 1d8 damage plus the attack bonus (both the Basic statistic and the additional bonus).
When you gain this Style, describe a new Experience (ideally related in some way to the character's combat abilities this style represents, for example related to knowledge of weapons, combat tactics, etc.); this Experience has a bonus equal to the attack bonus (not including the Basic Statistic) granted by this style.
\\
\underline{Upgrades:}
\begin{itemize}
  \item[-] The damage die can be changed to a d10 for 5 XP.
  \item[-] The range can be changed to \emph{just outside the reach of a similarly sized creature} for 2 XP.
\end{itemize}

\paragraph{Ranged}
Choose a Basic Statistic as the base attack bonus; the character gains a $+1$ bonus to attacks with this style on top of the Basic Statistic's bonus.
The range of attacks with this style is \emph{within a reasonable distance}.
Attacks with this style deal 1d6 damage plus the attack bonus (both the Basic statistic and the additional bonus).
When you gain this Style, describe a new Experience (ideally related in some way to the character's combat abilities this style represents, for example related to knowledge of weapons, combat tactics, etc.); this Experience has a bonus equal to the attack bonus (not including the Basic Statistic) granted by this style.
\\
\underline{Upgrades:}
\begin{itemize}
  \item[-] The damage die be changed to a d8 for 5 XP.
  \item[-] The range can be changed to \emph{from an unreasonable distance} for 2 XP.
\end{itemize}

\paragraph{Simple \& Weak}
Choose a Basic Statistic as the base attack bonus; choose a \emph{Specialization} with the Simple \& Weak tag, the bonus to attacks using this style is the bonus to the Basic Statistic plus the bonus to this Specialization.
Because this Style utilizes a Simple \& Weak specialization, all attack rolls using this Style are at $-1$ Difficulty.
...
Attacks with this style deal 1d4 damage plus the attack bonus (both the Basic statistic and the Specialization's bonus).
\\
\underline{Upgrades:}
\begin{itemize}
  \item[-] The damage die be changed to a d6 for 5 XP.
...
Choose a Basic Statistic as the base attack bonus; choose a \emph{Specialization} with the Complex \& Powerful tag, the bonus to attacks using this style is the bonus to the Basic Statistic plus the bonus to this Specialization.
Because this Style utilizes a Complex \& Powerful specialization, all attack rolls using this Style are at $+1$ Difficulty.
The range of attacks with this style is \emph{from an unreasonable distance}, but for attacks from a long range a $-1$ penalty should be applied to the roll.
Attacks with this style deal 1d10 damage plus the attack bonus (both the Basic statistic and the Specialization's bonus).
\\
\underline{Upgrades:}
\begin{itemize}
  \item[-] The damage die be changed to a d12 for 5 XP.
  \item[-] The penalty for attacks at long range can be eliminated for 2 XP.
\end{itemize}

\subsubsection{Defensive Combat Styles}\index{Combat Style!Defensive}
\paragraph{Evasive}
...
When attacked, if the character does not use a Defend Action, they can still roll to defend but with a $-3$ penalty to the roll.

\paragraph{Shielded}
Choose a Basic Statistic as the base bonus to defend.
The character also gains a $+1$ bonus to Block when using this style.
\\
\underline{Upgrades}:
\begin{itemize}
  \item[-] The bonus to block can be increased (up to a maximum of $+2$ at character creation or $+5$ thereafter) for XP equal to $3\times$ the new bonus.
\end{itemize}

\paragraph{Riposte}
Choose a Basic Statistic as the base bonus to defend.
When using this style, if the character is within range of the attacker, they can deal 1d4 damage to the attacker, regardless of the result of the roll.
\\
\underline{Upgrades}:
\begin{itemize}
  \item[-] The damage can be increased to a d6 for 5 XP, to a d8 for 15 XP, to a d10 for 30 XP, or to a d12 for 50 XP (an existing die can be increased to the next higher die size for the difference in their XP costs).
\end{itemize}
...
A starting character also has a \emph{Wealth} of \emph{Limited}.
...
\begin{itemize}
  ...
  \item A character's \Speed{} can be increased to \emph{Fast} for 1 XP or can be reduced to \emph{Slow} to gain 1 XP.
  \item A character can add a Climb or Swim speed at \emph{Slow} for 6 XP, with it costing 1 XP to increase to \emph{Normal} and another 1 XP to increase to \emph{Fast}, a Burrow speed at \emph{Slow} for 10 XP (with increasing speed costing the same), or a Fly speed for 16 XP (with increases costing the same).
  ...
  \item To increase their \emph{Wealth} from Limited to Modest for 2 XP, from Modest to Comfortable for 6 XP, from Comfortable to Luxurious to 10 XP, or Luxurious to Aristocratic for 25 XP. A creature can also decrease their Wealth from Limited to None to gain 2 XP. 
\end{itemize}
...
\begin{itemize}
  ...
  \item Gain an \emph{Advanced Character Option} as described in Section~\ref{sec:character-advanced}.
\end{itemize}
